...
func get_llm_deck_strategy_prompt(game_state: GameState, player_index: int) -> PackedStringArray:
	return super.get_llm_deck_strategy_prompt(game_state, player_index)
...
func _should_suppress_replan_for_live_conversion(
	...
	var terminal_type := str(route_goal.get("terminal_type", ""))
	if _deck_snapshot_has_live_terminal_conversion(after_snapshot):
		if remaining_queue.is_empty():
...
func _prune_queue_to_live_terminal_conversion(
	...
	var deck_queue := _deck_pruned_live_terminal_conversion_queue(after_snapshot, remaining_queue)
...
func score_action_absolute(action: Dictionary, game_state: GameState, player_index: int) -> float:
	...
	if _is_low_value_runtime_attack_action(action, game_state, player_index) \
		...
		_audit_log("low_value_attack_runtime_blocked", {
			...
			"player_index": player_index,
			"action": _audit_logger.call("compact_action", action) if _audit_logger != null else action,
			"hand_count": game_state.players[player_index].hand.size() if game_state != null and player_index >= 0 and player_index < game_state.players.size() else -1,
			"deck_count": _player_deck_count_for_llm(game_state, player_index),
		})
		return -10000.0
...
func _append_short_route_followups(target: Array[Dictionary], seen_ids: Dictionary) -> void:
	_deck_append_short_route_followups(target, seen_ids)
...
func _candidate_route_fallback_tree() -> Dictionary:
	var route_id := _best_candidate_route_action_id_for_fallback()
...
func _best_candidate_route_action_id_for_fallback() -> String:
	var has_primary_route := _route_candidates_include_primary_progress()
	var best_id := ""
	...
	for raw_key: Variant in _llm_route_candidates_by_id.keys():
		...
		if route.is_empty():
			continue
		if has_primary_route and _candidate_route_is_low_value_active_attach(route):
			continue
...
func _candidate_route_base_priority(route: Dictionary) -> int:
	if route.has("priority"):
		return int(route.get("priority", 0))
	return int(route.get("base_priority", 0))


func _route_candidates_include_primary_progress() -> bool:
	for raw_key: Variant in _llm_route_candidates_by_id.keys():
		var route: Dictionary = _llm_route_candidates_by_id.get(raw_key, {}) if _llm_route_candidates_by_id.get(raw_key, {}) is Dictionary else {}
		if _candidate_route_advances_primary_attack(route):
			return true
	return false


func _candidate_route_advances_primary_attack(route: Dictionary) -> bool:
	...
	for raw_goal: Variant in route.get("future_goals", []):
		if not (raw_goal is Dictionary):
			continue
		var goal_ref: Dictionary = raw_goal
		var quality: Dictionary = goal_ref.get("attack_quality", {}) if goal_ref.get("attack_quality", {}) is Dictionary else {}
		if str(quality.get("role", "")) == "primary_damage" or str(quality.get("terminal_priority", "")) == "high":
			return true
	return false


func _candidate_route_is_low_value_active_attach(route: Dictionary) -> bool:
	var route_id := str(route.get("id", route.get("route_action_id", "")))
	if not route_id.contains("manual_attach_to_active_attack"):
		return false
	var saw_attack_goal := false
	for raw_goal: Variant in route.get("future_goals", []):
		if not (raw_goal is Dictionary):
			continue
		var goal: Dictionary = raw_goal
		if str(goal.get("attack_name", "")).strip_edges() != "" or goal.has("attack_quality"):
			saw_attack_goal = true
		var quality: Dictionary = goal.get("attack_quality", {}) if goal.get("attack_quality", {}) is Dictionary else {}
		if str(quality.get("terminal_priority", "")) == "low":
			return true
		if _is_low_value_first_attack_ref(goal):
			return true
	return not saw_attack_goal
...
func _action_array_contains_attack_setup(raw_actions: Variant) -> bool:
	...
	for raw_action: Variant in raw_actions:
		...
		if action_type == "attach_energy" and position == "active":
			return true
		if _deck_action_ref_enables_attack(ref):
			return true
	return false
...
func _validate_action_interactions(action_id: String, ref: Dictionary, interactions: Dictionary, path: String, errors: Array[String]) -> void:
	...
	for raw_key: Variant in interactions.keys():
		...
		if not schema.has(key):
			errors.append("%s uses interaction key '%s' for '%s' but legal_action.interaction_schema does not expose that key" % [path, key, action_id])
			return
	_deck_validate_action_interactions(action_id, ref, interactions, path, errors)
...
func _deck_snapshot_has_live_terminal_conversion(_snapshot: Dictionary) -> bool:
	return false


func _deck_replan_trigger_after_state_change(_before_snapshot: Dictionary, _after_snapshot: Dictionary, _context: Dictionary) -> Dictionary:
	return {"should_replan": false}
...
func _deck_can_replace_end_turn_with_action(_action: Dictionary, _game_state: GameState, _player_index: int) -> bool:
	return false


func _deck_should_block_exact_queue_match(_queued_action: Dictionary, _runtime_action: Dictionary, _game_state: GameState, _player_index: int) -> bool:
	return false


func _deck_queue_item_matches_action(_queued_action: Dictionary, _runtime_action: Dictionary, _game_state: GameState, _player_index: int) -> bool:
	return false


func _deck_action_ref_enables_attack(_ref: Dictionary) -> bool:
	return false


func _deck_validate_action_interactions(_action_id: String, _ref: Dictionary, _interactions: Dictionary, _path: String, _errors: Array[String]) -> void:
	pass


func _deck_preferred_terminal_attack_for(_action: Dictionary, _game_state: GameState, _player_index: int) -> Dictionary:
	return {}


func _deck_is_low_value_runtime_attack_action(_action: Dictionary, _game_state: GameState, _player_index: int) -> bool:
	return false


func _deck_hand_card_is_productive_piece(_card_data: CardData) -> bool:
	return false


func _deck_is_high_pressure_attack_action(_action: Dictionary, _game_state: GameState, _player_index: int) -> bool:
	return false


func _deck_estimate_multiplier_attack_damage(_action: Dictionary, _game_state: GameState, _player_index: int, _base_damage: int, _lower_text: String) -> int:
	return 0
...
func _deck_hand_has_recovery_or_pivot_piece(_player: PlayerState) -> bool:
	return false


func _deck_append_short_route_followups(_target: Array[Dictionary], _seen_ids: Dictionary) -> void:
	pass


func _deck_append_productive_engine_candidates(
	_target: Array[Dictionary],
	_seen_ids: Dictionary,
...
func _preferred_terminal_attack_for(
	...
	var deck_preferred := _deck_preferred_terminal_attack_for(action, game_state, player_index)
	if not deck_preferred.is_empty():
		return _copy_attack_ref_with_interactions(deck_preferred, action)
...
func _is_low_value_first_attack_ref(action: Dictionary) -> bool:
	...
	if attack_rules is Dictionary:
		...
		if damage == "" and text.contains("discard") and text.contains("hand") and text.contains("draw"):
			return true
...
func _should_block_llm_end_turn(game_state: GameState, player_index: int) -> bool:
	if _deck_should_block_end_turn(game_state, player_index):
		return true
...
func _should_block_low_value_llm_attack_ref(ref: Dictionary, game_state: GameState, player_index: int) -> bool:
	if not _is_attack_action_ref(ref):
		return false
	if not _is_low_value_attack_ref_by_quality(ref):
		return false
	return _should_block_low_value_runtime_attack_context(game_state, player_index)


func _should_block_low_value_runtime_attack_context(game_state: GameState, player_index: int) -> bool:
	if _catalog_has_reachable_primary_future_attack():
		return true
	if game_state == null or player_index < 0 or player_index >= game_state.players.size():
		return false
	var player: PlayerState = game_state.players[player_index]
...
func _is_low_value_runtime_attack_action(action: Dictionary, game_state: GameState, player_index: int) -> bool:
	...
	var text := str(attack.get("text", "")).to_lower()
	if _deck_is_low_value_runtime_attack_action(action, game_state, player_index):
		return true
	if _name_contains(attack_name, "Roar") or _name_contains(attack_name, "Bursting Roar") or _name_contains(attack_name, "Burst Roar"):
		return true
	return text.contains("discard") and text.contains("hand") and text.contains("draw")


func _hand_has_non_energy_productive_piece(player: PlayerState) -> bool:
	if player == null:
		return false
	for card: CardInstance in player.hand:
		if card == null or card.card_data == null:
		...
		if cd.is_pokemon():
			return true
		if _deck_hand_card_is_productive_piece(cd):
			return true
		if _name_contains(name_en, "Nest Ball") \
				...
				or _name_contains(name_en, "Bravery Charm") \
				or _name_contains(name_en, "Iono"):
			return true
	return false


func _catalog_has_productive_non_terminal_action() -> bool:
	for raw_key: Variant in _llm_action_catalog.keys():
		var ref: Dictionary = _llm_action_catalog.get(raw_key, {})
...
func _is_current_action_high_pressure_attack_ref(action: Dictionary, game_state: GameState, player_index: int) -> bool:
	if not _is_attack_action_ref(action):
		return false
	if _deck_is_high_pressure_attack_action(action, game_state, player_index):
		return true
...
func _estimated_current_attack_damage(action: Dictionary, game_state: GameState, player_index: int) -> int:
	...
	if _runtime_text_looks_like_both_active_energy_bonus(lower):
		...
		var deck_damage := _deck_estimate_multiplier_attack_damage(action, game_state, player_index, base_damage, lower)
		if deck_damage > 0:
			damage = deck_damage
		else:
		...
		if kind in ["attack", "granted_attack"]:
			...
			if card is CardInstance and (card as CardInstance).card_data != null:
				...
				if str(cd.card_type) == "Supporter":
					has_supporter = true
				if _deck_is_setup_or_resource_card(cd) \
...
func _fire_llm_request(game_state: GameState, player_index: int, legal_actions: Array = []) -> void:
	...
	if game_manager == null:
		...
		return
	var api_config: Dictionary = _llm_api_config_from_manager(game_manager)
	...
	_audit_log("request_fired", {
		...
		"config_source_path": str(api_config.get("config_source_path", "")),
		...
		"future_actions": payload.get("future_actions", []),
		"turn_tactical_facts": payload.get("turn_tactical_facts", {}),
		"candidate_routes": payload.get("candidate_routes", []),
		"deck_strategy_hints": payload.get("deck_strategy_hints", []),
...
func _llm_api_config_from_manager(game_manager: Variant) -> Dictionary:
	if game_manager != null and game_manager.has_method("get_llm_opponent_battle_review_api_config"):
		return game_manager.call("get_llm_opponent_battle_review_api_config")
	if game_manager != null and game_manager.has_method("get_battle_review_api_config"):
		return game_manager.call("get_battle_review_api_config")
	return {}
