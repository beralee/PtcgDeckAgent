class_name AttackChooseOpponentBenchDamageCounters
extends BaseEffect

const STEP_ID := "opponent_bench_damage_counter_target"
...
func build_ucis_attack_interaction_steps_spec_steps(
	...
	if card == null or state == null or not applies_to_attack_index(_resolve_attack_index(card, attack)):
		return []
	var legal_targets := _legal_targets(card.owner_index, state)
	...
	return [{
		"id": STEP_ID,
		"title": "选择对手的1只备战宝可梦，放置%d个伤害指示物" % counter_count,
		"items": legal_targets,
		"labels": labels,
		"min_select": 1,
		"max_select": 1,
...
func validate_attack_interaction(
	...
	if attacker == null or state == null or not applies_to_attack_index(attack_index):
		return interaction_validation_error("bench damage-counter attacker or attack index is invalid")
	var top := attacker.get_top_card()
	if top == null:
		return interaction_validation_error("bench damage-counter attacker owner is invalid")
	var legal_targets := _legal_targets(top.owner_index, state)
	if legal_targets.is_empty():
...
func execute_attack(
	...
	var legal_targets: Array = opponent.bench.duplicate()
	if legal_targets.is_empty():
		return
...
func _legal_targets(owner_index: int, state: GameState) -> Array:
	if owner_index < 0 or owner_index >= state.players.size():
		return []
	return state.players[1 - owner_index].bench.duplicate()
...
func get_description() -> String:
	return "Put %d damage counters on 1 of your opponent's Benched Pokemon." % counter_count
