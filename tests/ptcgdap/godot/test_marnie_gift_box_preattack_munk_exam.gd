class_name TestMarnieGiftBoxPreattackMunkExam
extends TestBase

const CORPUS_PATH := (
	"res://tests/ptcgdap/exams/marnie_gift_box_match_20260829_165713_715386_r54.json"
)
const EXAM_ID := "t7_bench_munkidori_after_froslass_before_nonterminal_attack"
const HarnessScript = preload(
	"res://tests/ptcgdap/godot/support/MarnieGiftBoxReplayExamHarness.gd"
)


func test_froslass_engine_deploys_munkidori_before_attacking() -> String:
	...
	return run_checks([
		...
		assert_true(bool(report.get("all_passed", false)), "Munkidori deployment mismatch: %s" % report),
	])
