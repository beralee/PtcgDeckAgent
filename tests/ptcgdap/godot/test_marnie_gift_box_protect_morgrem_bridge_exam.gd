class_name TestMarnieGiftBoxProtectMorgremBridgeExam
extends TestBase

const CORPUS_PATH := (
	"res://tests/ptcgdap/exams/marnie_gift_box_match_20260829_165713_715386_r54.json"
)
const EXAM_ID := "t3_retreat_morgrem_to_impidimp_and_preserve_grimmsnarl_bridge"
const HarnessScript = preload(
	"res://tests/ptcgdap/godot/support/MarnieGiftBoxReplayExamHarness.gd"
)


func test_retreat_morgrem_to_preserve_grimmsnarl_bridge() -> String:
	...
	return run_checks([
		...
		assert_true(
			...
			"Morgrem bridge-preservation mismatch: %s" % report
		),
	])
