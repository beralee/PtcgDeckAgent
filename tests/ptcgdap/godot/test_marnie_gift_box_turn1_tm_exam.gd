class_name TestMarnieGiftBoxTurn1TmExam
extends TestBase

const CORPUS_PATH := (
	"res://tests/ptcgdap/exams/marnie_gift_box_match_20260829_165713_715386_r54.json"
)
const EXAM_ID := "t1_first_player_preserves_tm_evolution_when_attack_is_illegal"
const HarnessScript = preload(
	"res://tests/ptcgdap/godot/support/MarnieGiftBoxReplayExamHarness.gd"
)


func test_first_player_preserves_tm_until_an_attack_is_legal() -> String:
	...
	return run_checks([
		...
		assert_true(bool(report.get("all_passed", false)), "TM preservation mismatch: %s" % report),
	])
