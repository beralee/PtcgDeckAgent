...
SNAPSHOT_ROOT = ROOT / "artifacts/ptcgdap/as_wp0/parent_snapshot"
MANIFEST_PATH = SNAPSHOT_ROOT / "manifest.json"
PARENT_MANIFEST_PATH = ROOT / "artifacts/ptcgdap/p5_wp7/manifest.json"
PORTABLE_POLICY_BUNDLE_PATH = ROOT / "contracts/ptcgdap/marnie_portable_policy_bundle.json"
SOURCE_LOCK_PATH = ROOT / "docs/ptcgdap/SOURCE_LOCK.json"
SUCCESSOR_MANIFEST_PATH = ROOT / "artifacts/ptcgdap/as_wp1/parent_snapshot/manifest.json"
SUCCESSOR_SUPPLEMENTAL_ROOT = ROOT / "artifacts/ptcgdap/as_wp1/supplemental_parent_snapshot"
SUCCESSOR_SUPPLEMENTAL_MANIFEST_PATH = SUCCESSOR_SUPPLEMENTAL_ROOT / "manifest.json"
LATEST_SUCCESSOR_ROOT = ROOT / "artifacts/ptcgdap/as_wp2/parent_snapshot"
LATEST_SUCCESSOR_MANIFEST_PATH = LATEST_SUCCESSOR_ROOT / "manifest.json"
FINAL_SUCCESSOR_MANIFEST_PATH = ROOT / "artifacts/ptcgdap/as_wp3/parent_snapshot/manifest.json"
...
EXPECTED_MANIFEST_RAW = "DB0F242F3A3BA79F85439A3AF7C272B713391FB562ABF708C034611458DD63BB"
EXPECTED_MANIFEST_CANONICAL = "ED93C74FA56D254AF6874808C683DB689A8AF0C1AEB3A7DCE384E4A254890C31"
...
class AsWp0ParentSnapshotTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        if EXPECTED_MANIFEST_RAW != sha(MANIFEST_PATH.read_bytes()):
            raise AssertionError("AS-WP0 snapshot raw drift")
        cls.manifest = load_json_strict(MANIFEST_PATH)
        if EXPECTED_MANIFEST_CANONICAL != sha(canonical_json_v1_bytes(cls.manifest)):
            raise AssertionError("AS-WP0 snapshot canonical drift")
        cls.entries = {safe_path(entry["original_path"]): entry for entry in cls.manifest["files"]}

    def test_payloads_are_exact_safe_complete_and_bind_sealed_parent(self) -> None:
        self.assertEqual("AS-WP0", self.manifest["work_package"])
        self.assertEqual(7, self.manifest["file_count"])
        ...
        self.assertEqual(
            EXPECTED_PORTABLE_POLICY_BUNDLE,
            sha(canonical_json_v1_bytes(load_json_strict(PORTABLE_POLICY_BUNDLE_PATH))),
        )
        self.assertEqual(
            EXPECTED_SOURCE_LOCK,
            sha(canonical_json_v1_bytes(load_json_strict(SOURCE_LOCK_PATH))),
        )
        scope = self.manifest["rollback_scope"]
        self.assertEqual(set(self.entries), set(scope["restore_exact_paths"]))
        self.assertEqual(3, len(set(scope["delete_exact_paths"])))
        self.assertFalse(set(self.entries) & set(scope["delete_exact_paths"]))
        self.assertEqual("artifacts/ptcgdap/as_wp0/", scope["delete_evidence_prefix"])
    ...
    def test_isolated_restore_contains_only_parent_files(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ptcgdap-as-wp0-parent-") as temp:
    ...
    def test_virtual_rollback_reproduces_pre_as_wp0_candidate(self) -> None:
        ...
        successor_supplemental = load_json_strict(SUCCESSOR_SUPPLEMENTAL_MANIFEST_PATH)
        successor_parent_bytes = {
            safe_path(entry["original_path"]): decode(entry, SUCCESSOR_SUPPLEMENTAL_ROOT)
            for entry in successor_supplemental["files"]
        }
        ...
        latest_successor = load_json_strict(LATEST_SUCCESSOR_MANIFEST_PATH)
        latest_scope = latest_successor["rollback_scope"]
        latest_parent_bytes = {
        ...
        latest_delete_prefixes = (
            latest_scope["delete_evidence_prefix"],
            latest_scope["delete_fixture_prefix"],
        )
        final_scope = load_json_strict(FINAL_SUCCESSOR_MANIFEST_PATH)["rollback_scope"]
        final_delete_paths = set(final_scope["delete_exact_paths"])
        final_omit_clean_paths = set(final_scope["omit_parent_clean_paths"])
        final_delete_prefix = final_scope["delete_evidence_prefix"]
        ...
        for code, path in status():
            ...
            if (
                ...
                or path.startswith(final_delete_prefix)
                or path in final_delete_paths
                or path in final_omit_clean_paths
                or path.startswith(latest_delete_prefixes)
            ...
            records.append(
                restore_pre_author_player_owner_record("AS-WP0", {
                    "path": path,
                    "status": code,
