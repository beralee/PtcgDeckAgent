...
SNAPSHOT_ROOT = ROOT / "artifacts/ptcgdap/p3_wp3/parent_snapshot"
MANIFEST_PATH = SNAPSHOT_ROOT / "manifest.json"
EXPECTED_MANIFEST_RAW = "E3BA39FF422E7F37A94ACE7E1A11FA64D06353AD7087826F2002FCBADD1F83CD"
EXPECTED_MANIFEST_CANONICAL = "C3E02553BA70DB417C3D14510C979DA1AC081D44B276DEDC699B6DC9BBFE5957"
EXPECTED_PARENT_MANIFEST_RAW = "342DDC34D8EB95DBB9253944B871F6AB3433A0E14FE03BBA3A9714E72B9019B1"
EXPECTED_PARENT_MANIFEST_CANONICAL = "F1ABB0D82469174C321DA42D54AC24F045908183898F16F1A80E97BE4277BA0E"
EXPECTED_PARENT_DIGEST = "8E24475490BCB5033645E861F07FABC7EF69FFBDF365D593798BCBD231619556"
EXPECTED_COUNTS = {"untracked": 380, "deleted": 28, "modified": 1}
P3_WP3_PATHS = {
...
P3_WP4_PREFIX = "artifacts/ptcgdap/p3_wp4/"
P3_WP4_PATHS = {
...
class P3Wp3ParentSnapshotTests(unittest.TestCase):
    ...
    def test_payloads_are_exact_safe_and_complete(self) -> None:
        self.assertEqual(self.manifest["work_package"], "P3-WP3")
        self.assertEqual(self.manifest["file_count"], 19)
        ...
        self.assertEqual(set(self.manifest["rollback_scope"]["delete_exact_paths"]), P3_WP3_PATHS)
    ...
    def test_isolated_restore_contains_only_parent_files(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ptcgdap-p3-wp3-parent-") as temp:
            root = Path(temp)
    ...
    def test_virtual_rollback_reproduces_p3_wp2_candidate(self) -> None:
        ...
        for code, path in status():
            if path.startswith("artifacts/ptcgdap/p3_wp6/") or path in P3_WP6_PATHS or path.startswith("artifacts/ptcgdap/p3_wp7/") or path in P3_WP7_PATHS or path.startswith("artifacts/ptcgdap/p3_wp8/") or path in P3_WP8_PATHS or path.startswith("artifacts/ptcgdap/p4_wp1/") or path in P4_WP1_PATHS or path.startswith("artifacts/ptcgdap/p4_wp2/") or path in P4_WP2_PATHS or path.startswith("artifacts/ptcgdap/p4_wp3/") or path in P4_WP3_PATHS or path.startswith("artifacts/ptcgdap/p4_wp4/") or path in P4_WP4_PATHS or path.startswith("artifacts/ptcgdap/p4_wp5/") or path in P4_WP5_PATHS or path.startswith("artifacts/ptcgdap/p4_wp6/") or path in P4_WP6_PATHS:
                continue
            if (
                path.startswith("artifacts/ptcgdap/p3_wp2/")
                or path.startswith("artifacts/ptcgdap/p3_wp3/")
                or path in P3_WP3_PATHS
                or path.startswith(P3_WP4_PREFIX)
                or path in P3_WP4_PATHS
                or path.startswith(P3_WP5_PREFIX)
                or path in P3_WP5_PATHS
            ):
                continue
