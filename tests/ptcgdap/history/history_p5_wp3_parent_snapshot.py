...
SNAPSHOT_ROOT = ROOT / "artifacts/ptcgdap/p5_wp3/parent_snapshot"
MANIFEST_PATH = SNAPSHOT_ROOT / "manifest.json"
PARENT_MANIFEST_PATH = ROOT / "artifacts/ptcgdap/p5_wp2/manifest.json"
EXPECTED_PARENT_MANIFEST_RAW = "2A3827A51E4500C545684B0895996DD35475F5FCF6F9D0EDCD09F0CF0A16E83E"
EXPECTED_PARENT_MANIFEST_CANONICAL = "EB6A0513D34116F16408F2E6293D7C62B6F12C5472018CDB3C3EDAA5D8E1608F"
EXPECTED_PARENT_DIGEST = "4F45C4B548DB03BA881F489C2EB54DA5B19BCF9AEAF13C4B96F240BD408F7DDF"
EXPECTED_COUNTS = {"untracked": 1095, "deleted": 28, "modified": 1}
EXPECTED_MANIFEST_RAW = "6AACAB27372FE94D34CC2E031BC4F0E8B632879AFE111DB3FBD9C2C6D4ECF3D1"
EXPECTED_MANIFEST_CANONICAL = "A3AF038DB6C92228C6E2A36B53023094C7BFD2AA837DB597DA6A66DB59C8EC5E"
P5_WP3_PATHS = frozenset(
    load_json_strict(MANIFEST_PATH)["rollback_scope"]["delete_exact_paths"]
)


def is_p5_wp3_path(path: str) -> bool:
    if path.startswith("artifacts/ptcgdap/p5_wp3/") or path in P5_WP3_PATHS:
        return True
    try:
        from tests.ptcgdap.test_p5_wp4_parent_snapshot import is_p5_wp4_path
    except ImportError:
        return False
    return is_p5_wp4_path(path)


def p5_wp4_parent_bytes(path: str, current: bytes) -> bytes:
    """Return the exact P5-WP3 byte for a path amended by P5-WP4."""
    try:
        from tests.ptcgdap.test_p5_wp5_parent_snapshot import p5_wp5_parent_bytes
    except ImportError:
        pass
    else:
        current = p5_wp5_parent_bytes(path, current)
    try:
        from tests.ptcgdap.test_p5_wp4_parent_snapshot import MANIFEST_PATH as p5_wp4_manifest, decode as decode_p5_wp4
    except ImportError:
        return current
    if not p5_wp4_manifest.is_file():
        return current
    for entry in load_json_strict(p5_wp4_manifest)["files"]:
        if entry["original_path"] == path:
            return decode_p5_wp4(entry)
    return current
...
class P5Wp3ParentSnapshotTests(unittest.TestCase):
    ...
    def test_payloads_are_exact_safe_complete_and_bind_parent(self) -> None:
        self.assertEqual(41, self.manifest["file_count"])
        self.assertEqual(41, len(self.entries))
        ...
        self.assertEqual("artifacts/ptcgdap/p5_wp3/", scope["delete_evidence_prefix"])
    ...
    def test_isolated_restore_contains_only_parent_files(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ptcgdap-p5-wp2-parent-") as temp:
    ...
    def test_virtual_rollback_reproduces_p5_wp2_candidate(self) -> None:
        ...
        for code, path in status():
            if path.startswith("artifacts/ptcgdap/p5_wp2/") or is_p5_wp3_path(path) or path in delete_paths:
                continue
        ...
        self.assertEqual(1124, len(records))
