from __future__ import annotations

import base64
from collections import Counter
import hashlib
from pathlib import Path, PurePosixPath
import subprocess
import tempfile
import unittest

from scripts.ai.ptcgdap.source_lock import canonical_json_v1_bytes, load_json_strict


ROOT = Path(__file__).resolve().parents[2]
SNAPSHOT_ROOT = ROOT / "artifacts/ptcgdap/p5_wp5/parent_snapshot"
MANIFEST_PATH = SNAPSHOT_ROOT / "manifest.json"
SUPPLEMENTAL_ROOT = ROOT / "artifacts/ptcgdap/p5_wp5/supplemental_parent_snapshot"
SUPPLEMENTAL_MANIFEST_PATH = SUPPLEMENTAL_ROOT / "manifest.json"
PARENT_MANIFEST_PATH = ROOT / "artifacts/ptcgdap/p5_wp4/manifest.json"
EXPECTED_PARENT_MANIFEST_RAW = "925E0FC61B2A9613D3B163CC895E522E91E05CFB4DAC14A4864463C10486FB17"
EXPECTED_PARENT_MANIFEST_CANONICAL = "D1643D75D34BF235ECE401B2453AC413A3D1D94C40B491D47FFD48520F3AEDBB"
EXPECTED_PARENT_DIGEST = "D4EDB2C40B0D0CD1EAE97B42FAC58DA974E33A6CD95682BBBB1638F30A23A441"
EXPECTED_COUNTS = {"untracked": 1231, "deleted": 28, "modified": 1}
EXPECTED_MANIFEST_RAW = "6382E02AC38805CD167B749320395E36F762D3DC340DF854DF4F3C9FB8F3D4B7"
EXPECTED_MANIFEST_CANONICAL = "64F290725BF2441E9D08CED66AC727614098C4594945C9119F61D550943287F3"
EXPECTED_SUPPLEMENTAL_MANIFEST_RAW = "75B73F9E3CC3E0FFB419501E5005B7CC496D0A428AA3CC4820915C7B41ECA00D"
EXPECTED_SUPPLEMENTAL_MANIFEST_CANONICAL = "476F93C2D168776B9B9B7EC97E7246FD8F5C651E3E635067E4515D30271F798D"


def sha(value: bytes) -> str:
    return hashlib.sha256(value).hexdigest().upper()


def safe_path(raw: object) -> str:
    if not isinstance(raw, str) or not raw or "\\" in raw or "\0" in raw:
        raise AssertionError(raw)
    path = PurePosixPath(raw)
    if path.is_absolute() or "." in path.parts or ".." in path.parts or ":" in path.parts[0] or path.as_posix() != raw:
        raise AssertionError(raw)
    return raw


def _manifest() -> dict[str, object]:
    return load_json_strict(MANIFEST_PATH)


def is_p5_wp5_path(path: str) -> bool:
    try:
        from tests.ptcgdap.test_p5_wp6_parent_snapshot import is_p5_wp6_path
    except (ImportError, FileNotFoundError):
        pass
    else:
        if is_p5_wp6_path(path):
            return True
    if path.startswith("artifacts/ptcgdap/p5_wp5/"):
        return True
    if not MANIFEST_PATH.is_file():
        return False
    return path in frozenset(_manifest()["rollback_scope"]["delete_exact_paths"])


def decode(entry: dict[str, object], root: Path = SNAPSHOT_ROOT) -> bytes:
    name = safe_path(entry["snapshot_path"])
    if "/" in name:
        raise AssertionError(name)
    source = (root / name).read_bytes()
    if not source.endswith(b"\n") or b"\r" in source:
        raise AssertionError(name)
    lines = source[:-1].split(b"\n")
    if not lines or any(not line or len(line) > 4096 for line in lines) or any(len(line) != 4096 for line in lines[:-1]):
        raise AssertionError(name)
    encoded = b"".join(lines)
    value = base64.b64decode(encoded.decode("ascii"), validate=True)
    if base64.b64encode(value) != encoded:
        raise AssertionError(name)
    if len(source) != entry["snapshot_file_bytes"] or sha(source) != entry["snapshot_file_raw_sha256"]:
        raise AssertionError(name)
    if len(value) != entry["bytes"] or sha(value) != entry["raw_sha256"]:
        raise AssertionError(name)
    return value


def p5_wp5_parent_bytes(path: str, current: bytes) -> bytes:
    """Return the exact P5-WP4 byte for a path amended by P5-WP5."""
    try:
        from tests.ptcgdap.test_p5_wp6_parent_snapshot import p5_wp6_parent_bytes
    except (ImportError, FileNotFoundError):
        pass
    else:
        current = p5_wp6_parent_bytes(path, current)
    if not MANIFEST_PATH.is_file():
        return current
    for entry in _manifest()["files"]:
        if entry["original_path"] == path:
            return decode(entry)
    if SUPPLEMENTAL_MANIFEST_PATH.is_file():
        for entry in load_json_strict(SUPPLEMENTAL_MANIFEST_PATH)["files"]:
            if entry["original_path"] == path:
                return decode(entry, SUPPLEMENTAL_ROOT)
    return current


def status() -> list[tuple[str, str]]:
    output = subprocess.run(
        ["git", "status", "--porcelain=v1", "-z", "--untracked-files=all"],
        cwd=ROOT, check=True, capture_output=True,
    ).stdout
    result = []
    for record in output.split(b"\0"):
        if not record:
            continue
        code = record[:2].decode("ascii")
        path = safe_path(record[3:].decode("utf-8").replace("\\", "/"))
        if "R" in code or "C" in code:
            raise AssertionError("rename/copy outside snapshot algorithm")
        result.append((code, path))
    return result


class P5Wp5ParentSnapshotTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        raw = MANIFEST_PATH.read_bytes()
        if sha(raw) != EXPECTED_MANIFEST_RAW:
            raise AssertionError("P5-WP5 snapshot raw drift")
        cls.manifest = load_json_strict(MANIFEST_PATH)
        if sha(canonical_json_v1_bytes(cls.manifest)) != EXPECTED_MANIFEST_CANONICAL:
            raise AssertionError("P5-WP5 snapshot canonical drift")
        cls.entries = {safe_path(entry["original_path"]): entry for entry in cls.manifest["files"]}
        supplemental_raw = SUPPLEMENTAL_MANIFEST_PATH.read_bytes()
        if sha(supplemental_raw) != EXPECTED_SUPPLEMENTAL_MANIFEST_RAW:
            raise AssertionError("P5-WP5 supplemental snapshot raw drift")
        cls.supplemental = load_json_strict(SUPPLEMENTAL_MANIFEST_PATH)
        if sha(canonical_json_v1_bytes(cls.supplemental)) != EXPECTED_SUPPLEMENTAL_MANIFEST_CANONICAL:
            raise AssertionError("P5-WP5 supplemental snapshot canonical drift")
        cls.supplemental_entries = {
            safe_path(entry["original_path"]): entry for entry in cls.supplemental["files"]
        }

    def test_payloads_are_exact_safe_complete_and_bind_parent(self) -> None:
        self.assertEqual(24, self.manifest["file_count"])
        self.assertEqual(24, len(self.entries))
        parent = self.manifest["parent"]
        self.assertEqual(EXPECTED_PARENT_MANIFEST_RAW, parent["manifest_raw_sha256"])
        self.assertEqual(EXPECTED_PARENT_MANIFEST_CANONICAL, parent["manifest_canonical_sha256"])
        self.assertEqual(EXPECTED_PARENT_DIGEST, parent["worktree_snapshot_canonical_sha256"])
        self.assertEqual(EXPECTED_COUNTS, parent["worktree_status_counts"])
        self.assertEqual(EXPECTED_PARENT_MANIFEST_RAW, sha(PARENT_MANIFEST_PATH.read_bytes()))
        scope = self.manifest["rollback_scope"]
        self.assertEqual(set(self.entries), set(scope["restore_exact_paths"]))
        self.assertEqual(15, len(set(scope["delete_exact_paths"])))
        self.assertFalse(set(self.entries) & set(scope["delete_exact_paths"]))
        self.assertEqual("artifacts/ptcgdap/p5_wp5/", scope["delete_evidence_prefix"])
        names = [entry["snapshot_path"] for entry in self.entries.values()]
        self.assertEqual(len(names), len(set(names)))
        self.assertEqual({"manifest.json", *names}, {path.name for path in SNAPSHOT_ROOT.iterdir() if path.is_file()})
        for entry in self.entries.values():
            decode(entry)
        self.assertEqual(1, self.supplemental["file_count"])
        self.assertEqual(1, len(self.supplemental_entries))
        self.assertFalse(set(self.entries) & set(self.supplemental_entries))
        self.assertEqual(
            {"manifest.json", *[entry["snapshot_path"] for entry in self.supplemental_entries.values()]},
            {path.name for path in SUPPLEMENTAL_ROOT.iterdir() if path.is_file()},
        )
        for entry in self.supplemental_entries.values():
            decode(entry, SUPPLEMENTAL_ROOT)

    def test_isolated_restore_contains_only_parent_files(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ptcgdap-p5-wp4-parent-") as temp:
            restore_root = Path(temp)
            for path, entry in self.entries.items():
                destination = restore_root / path
                destination.parent.mkdir(parents=True, exist_ok=True)
                destination.write_bytes(decode(entry))
            for path, entry in self.supplemental_entries.items():
                destination = restore_root / path
                destination.parent.mkdir(parents=True, exist_ok=True)
                destination.write_bytes(decode(entry, SUPPLEMENTAL_ROOT))
            self.assertEqual(
                set(self.entries) | set(self.supplemental_entries),
                {path.relative_to(restore_root).as_posix() for path in restore_root.rglob("*") if path.is_file()},
            )

    def test_virtual_rollback_reproduces_p5_wp4_candidate(self) -> None:
        from tests.ptcgdap.test_p5_wp6_parent_snapshot import is_p5_wp6_path, p5_wp6_parent_bytes

        parent_bytes = {path: decode(entry) for path, entry in self.entries.items()}
        parent_bytes.update({path: decode(entry, SUPPLEMENTAL_ROOT) for path, entry in self.supplemental_entries.items()})
        delete_paths = set(self.manifest["rollback_scope"]["delete_exact_paths"])
        records = []
        restored = set()
        for code, path in status():
            if path.startswith("artifacts/ptcgdap/p5_wp4/") or path.startswith("artifacts/ptcgdap/p5_wp5/") or is_p5_wp6_path(path) or path in delete_paths:
                continue
            if path in parent_bytes:
                data = parent_bytes[path]
                restored.add(path)
            elif "D" in code:
                data = None
            else:
                data = p5_wp6_parent_bytes(path, (ROOT / path).read_bytes())
            records.append({"path": path, "status": code, "bytes": None if data is None else len(data), "sha256": None if data is None else sha(data)})
        self.assertEqual(set(parent_bytes), restored)
        records.sort(key=lambda item: item["path"])
        counts = Counter("untracked" if item["status"] == "??" else "deleted" if "D" in item["status"] else "modified" for item in records)
        self.assertEqual(1260, len(records))
        self.assertEqual(EXPECTED_COUNTS, dict(counts))
        self.assertEqual(EXPECTED_PARENT_DIGEST, sha(canonical_json_v1_bytes(records)))


if __name__ == "__main__":
    unittest.main()
