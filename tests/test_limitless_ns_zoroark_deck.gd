...
const KEY_CARD_REFS := [
	{"set": "LEN_JTG", "index": "97", "name_en": "N's Zorua", "name_zh": "N的索罗亚"},
	{"set": "LEN_JTG", "index": "98", "name_en": "N's Zoroark ex", "name_zh": "N的索罗亚克ex"},
	{"set": "LEN_JTG", "index": "116", "name_en": "N's Reshiram", "name_zh": "N的莱希拉姆"},
	{"set": "LEN_JTG", "index": "153", "name_en": "N's PP Up", "name_zh": "N的PP提升"},
]
...
func test_naic_limitless_ns_zoroark_deck_keeps_translated_cards() -> String:
	...
	var checks: Array[String] = [
		assert_not_null(deck, "NAIC N's Zoroark Limitless deck should load through CardDatabase.get_deck"),
		assert_eq(deck.deck_name, "NAIC2025 N的索罗亚克", "NAIC N's Zoroark deck display name should stay translated"),
		assert_eq(total_count, 60, "NAIC N's Zoroark Limitless deck should contain exactly 60 cards"),
		assert_str_contains(manifest, "res://data/bundled_user/decks/%d.json" % NAIC_NS_ZOROARK_DECK_ID, "Bundled manifest should seed the NAIC N's Zoroark deck JSON"),
	]
	for spec: Dictionary in KEY_CARD_REFS:
		var card_ref := "%s_%s" % [str(spec["set"]), str(spec["index"])]
		checks.append(assert_contains(refs, card_ref, "Deck should include key Limitless card %s" % card_ref))
		checks.append(assert_str_contains(manifest, "res://data/bundled_user/cards/%s.json" % card_ref, "Manifest should include key Limitless card %s" % card_ref))
...
func test_limitless_ns_zoroark_key_cards_build_and_are_implemented() -> String:
	...
	for spec: Dictionary in KEY_CARD_REFS:
		...
		if card != null:
			checks.append(assert_eq(str(card.name), str(spec["name_en"]), "Key card %s/%s should keep its English rule name" % [set_code, card_index]))
			checks.append(assert_eq(str(card.name_en), str(spec["name_en"]), "Key card %s/%s should keep its English name" % [set_code, card_index]))
			checks.append(assert_eq(str(card.name_zh), str(spec["name_zh"]), "Key card %s/%s should store its Chinese display name" % [set_code, card_index]))
			checks.append(assert_eq(card.display_name(), str(spec["name_zh"]), "Key card %s/%s should prefer Chinese display name" % [set_code, card_index]))
...
func test_limitless_ns_zoroark_deck_entries_display_translated_generated_cards() -> String:
	...
	for spec: Dictionary in KEY_CARD_REFS:
		...
		if not entry.is_empty():
			checks.append(assert_eq(str(entry.get("name", "")), str(spec["name_zh"]), "Deck entry should display translated name for %s/%s" % [spec["set"], spec["index"]]))
			checks.append(assert_eq(str(entry.get("name_en", "")), str(spec["name_en"]), "Deck entry should preserve English name for %s/%s" % [spec["set"], spec["index"]]))
			checks.append(assert_eq(str(entry.get("source_name", "")), str(spec["name_en"]), "Deck entry should preserve Limitless source name for %s/%s" % [spec["set"], spec["index"]]))
	return run_checks(checks)


func test_limitless_ns_zoroark_card_view_uses_translated_display_name() -> String:
	var card := CardDatabase.get_card("LEN_JTG", "98")
...
func test_limitless_ns_zoroark_generated_cards_have_bundled_images() -> String:
	...
	for spec: Dictionary in KEY_CARD_REFS:
		...
		var image := Image.new()
		var decode_err := image.load_png_from_buffer(FileAccess.get_file_as_bytes(expected_path))
		...
		checks.append(assert_eq(decode_err, OK, "Bundled image should decode as PNG for %s/%s" % [set_code, card_index]))
...
func test_bundled_limitless_ns_zoroark_entries_keep_source_audit_metadata() -> String:
	...
	for entry: Dictionary in deck.cards:
		...
		checks.append(assert_true(str(entry.get("resolved_via", "")) in ["exact_print", "same_print_group", "generated_limitless_card", "basic_energy"],
...
func test_limitless_ns_zoroark_strategy_registry_mapping() -> String:
	...
	return run_checks([
		assert_eq(strategy_id, "ns_zoroark", "DeckStrategyRegistry should map the Limitless deck id to ns_zoroark"),
		assert_not_null(strategy, "DeckStrategyRegistry should instantiate the N's Zoroark strategy"),
		assert_eq(str(strategy.get_strategy_id()) if strategy != null and strategy.has_method("get_strategy_id") else "", "ns_zoroark", "Instantiated strategy should report ns_zoroark"),
	])
