...
func test_csv8c_165_blissey_ex_moves_basic_energy_from_any_own_pokemon() -> String:
	var processor := EffectProcessor.new()
	var state := _make_state()
	var player: PlayerState = state.players[0]
	...
	var source := player.active_pokemon
	var target := player.bench[0]
	player.bench[1] = blissey
	source.attached_energy.clear()
	target.attached_energy.clear()
	blissey.attached_energy.clear()
	var basic_energy := CardInstance.create(_make_energy_data("Basic Psychic", "P"), 0)
	source.attached_energy.append(basic_energy)
	...
	var source_index := source_items.find(basic_energy)
	var source_target_index := target_items.find(source)
	var excluded_targets: Array = steps[0].get("source_exclude_targets", {}).get(source_index, []) if source_index >= 0 and not steps.is_empty() else []
	var execute_ok := processor.execute_ability_effect(blissey, 0, [{
		"energy_assignment": [{"source": basic_energy, "target": target}],
	}], state)

	return run_checks([
		assert_not_null(effect, "CSV8C_165 should register Happy Switch"),
		assert_true(source_index >= 0, "Happy Switch should expose Basic Energy attached to any own Pokemon, not only Blissey ex"),
		assert_true(source_target_index in excluded_targets, "Happy Switch should prevent moving an Energy back to its source Pokemon"),
		assert_true(execute_ok, "Happy Switch should execute with a Basic Energy from another own Pokemon"),
		assert_false(basic_energy in source.attached_energy, "Happy Switch should remove the selected Energy from its actual source Pokemon"),
		assert_contains(target.attached_energy, basic_energy, "Happy Switch should attach the selected Energy to another own Pokemon"),
	])
...
func test_csv8c_165_blissey_ex_attack_can_decline_optional_draw() -> String:
	var processor := EffectProcessor.new()
	var state := _make_state()
	var player: PlayerState = state.players[0]
	player.hand.clear()
	...
	var steps: Array[Dictionary] = processor.get_attack_interaction_steps_by_id(blissey_cd.effect_id, 0, attacker.get_top_card(), blissey_cd.attacks[0], state)
	processor.execute_attack_effect(attacker, 0, state.players[1].active_pokemon, state, [{
		"draw_to_hand_size_choice": ["skip"],
	}])

	return run_checks([
		assert_eq(steps.size(), 1, "CSV8C_165 Return should expose an optional draw prompt"),
		assert_eq(player.hand.size(), 2, "CSV8C_165 Return should allow the player to decline drawing"),
		assert_eq(player.deck.size(), 6, "CSV8C_165 Return should leave the deck unchanged when draw is declined"),
	])
