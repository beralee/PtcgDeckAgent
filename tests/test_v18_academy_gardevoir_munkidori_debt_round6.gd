class_name TestV18AcademyGardevoirMunkidoriDebtRound6
extends TestBase
...
func test_academy_contract_resolves_munkidori_before_nonfinal_scream_tail() -> String:
	...
	return run_checks([
		assert_true(bool(flags.get("munkidori_damage_transfer_debt", false)),
			"Academy Gardevoir must expose its Munkidori transfer debt"),
		assert_true(ability_score > attack_score,
			"Munkidori transfer must resolve before a non-final Scream Tail attack"),
	])
