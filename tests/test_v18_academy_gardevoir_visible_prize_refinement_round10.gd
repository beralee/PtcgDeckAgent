class_name TestV18AcademyGardevoirVisiblePrizeRefinementRound10
extends TestBase
...
func test_visible_scream_tail_prize_stops_refinement_churn() -> String:
	...
	var refinement_score := float(strategy.call("score_action_absolute_with_plan", {
	...
	return run_checks([
		assert_true(refinement_score <= -3500.0,
			"A visible Scream Tail prize must stop Refinement churn (score=%f)" % refinement_score),
		assert_true(attack_score > refinement_score,
			"The visible prize attack must outrank Refinement"),
	])
