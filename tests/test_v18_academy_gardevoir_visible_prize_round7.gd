class_name TestV18AcademyGardevoirVisiblePrizeRound7
extends TestBase
...
func test_visible_scream_tail_prize_stops_extra_psychic_embrace() -> String:
	...
	return run_checks([
		assert_true(embrace_score <= -3500.0,
			"A visible Scream Tail prize must stop extra Embrace damage (score=%f)" % embrace_score),
		assert_true(attack_score > embrace_score,
			"The visible prize attack must outrank another Psychic Embrace"),
	])
