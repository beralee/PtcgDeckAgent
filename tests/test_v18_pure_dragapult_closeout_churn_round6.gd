class_name TestV18PureDragapultCloseoutChurnRound6
extends TestBase
...
func test_closeout_ready_dragapult_attacks_before_ultra_ball_churn() -> String:
	...
	return run_checks([
		assert_true(ball_score <= -4000.0,
			"Two-Prize closeout must not churn Ultra Ball before a ready Phantom Dive (score=%f)" % ball_score),
		assert_true(attack_score > ball_score, "The ready closeout attack must outrank optional search churn"),
	])
