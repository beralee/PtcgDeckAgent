class_name TestV18PureDragapultCloseoutEvolveRound8
extends TestBase
...
func test_two_prize_closeout_does_not_evolve_backup_before_phantom_dive() -> String:
	...
	var evolve_score := float(strategy.call("score_action_absolute_with_plan", {
		"kind": "evolve", "card": drakloak, "target_slot": dreepy,
	}, state, 0, plan))
	...
	return run_checks([
		assert_true(evolve_score <= -3600.0,
			"Two-Prize closeout must not evolve a backup lane first (score=%f)" % evolve_score),
		assert_true(attack_score > evolve_score,
			"Ready Phantom Dive must outrank optional backup evolution"),
	])
