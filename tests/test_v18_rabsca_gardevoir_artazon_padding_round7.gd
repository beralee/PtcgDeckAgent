class_name TestV18RabscaGardevoirArtazonPaddingRound7
extends TestBase
...
func test_developed_four_bench_shell_does_not_spend_artazon_on_padding() -> String:
	...
	return assert_true(score <= -3400.0,
		"A four-bench shell with two Ralts lanes and an attacker must reserve its final slot (score=%f)" % score)
