class_name TestV18YanmegaFullBenchArtazonRound8
extends TestBase
...
func test_artazon_is_not_donated_to_the_opponent_with_a_full_bench() -> String:
	...
	return run_checks([
		assert_true(full_score <= -2500.0, "Artazon must not be placed into an empty Stadium zone when Yanmega's Bench is full (score=%f)" % full_score),
		assert_true(open_score >= full_score + 2000.0, "Artazon must return while Yanmega still has a usable Bench slot (open=%f full=%f)" % [open_score, full_score]),
	])
...
func _pokemon(name: String, stage: String, cost: String) -> CardData:
	...
	card.hp = 280 if stage == "Stage 1" else 100
