...
const DECK_SEED_PATH := "res://data/bundled_user/decks/800018497.json"
const MANIFEST_PATH := "res://scripts/ai/v18_cpg/profiles/generated_semantic_manifests/800018497.json"
const REPORT_PATH := "res://tmp/v18cpg/optimization21/800018497/complex_decision_scenarios.json"
...
func _initialize() -> void:
	...
	_check(int(_profile.get("deck_id", 0)) == DECK_ID, "production standard Gardevoir profile must load")
	_check(int(_manifest.get("deck_id", 0)) == DECK_ID, "standard Gardevoir semantic manifest must load")
	...
	_scenario_a_charm_before_exact_six_embraces()
	...
	if _failures.is_empty() and _rows.size() == 5:
		print("optimization21 800018497 complex decision scenarios: PASS (5/5)")
		quit(0)
		return
	for failure: String in _failures:
		push_error(failure)
	print("optimization21 800018497 complex decision scenarios: FAIL (%d)" % _failures.size())
	quit(1)


func _scenario_a_charm_before_exact_six_embraces() -> void:
	...
	var psychic_fuel: Array[CardInstance] = []
	for index: int in 6:
		psychic_fuel.append(_real_instance(PSYCHIC_UID, 0))
	state.players[0].active_pokemon = scream_tail
	state.players[0].bench = [gardevoir]
	state.players[0].discard_pile = psychic_fuel.duplicate()
	_fill_prizes(state.players[0], 2, "OWN_CHARM_PRIZE")
	state.players[1].active_pokemon = _real_target("Public 230 HP two-Prize target", 230, 2)
	...
	var observation := _observation(
		[
			_ability("ability:embrace-without-budget", "slot:gardevoir", GARDEVOIR_UID, true),
			_attach_tool("tool:charm-scream-tail", BRAVERY_CHARM_UID, "slot:scream-tail"),
			_end_turn("end:missed-roaring-line"),
		],
		_slot("slot:scream-tail", SCREAM_TAIL_UID, [], 0, 90, 90, 1),
		[_slot("slot:gardevoir", GARDEVOIR_UID, [], 0, 310, 310, 2)],
		_public_target("PUBLIC_230_HP_EX", 230, 2),
		[],
		20,
		2
	)
	observation["own"]["discard"] = [
		_psychic_energy(), _psychic_energy(), _psychic_energy(),
		_psychic_energy(), _psychic_energy(), _psychic_energy(),
	]
	observation["own"]["hand"] = [_card(BRAVERY_CHARM_UID)]
	var facts := _facts(false, false, false, 1, false, false, 0, 2)
	var frontier := _frontier(observation, {
		"ability:embrace-without-budget": 700.0,
		"tool:charm-scream-tail": 620.0,
		"end:missed-roaring-line": -900.0,
	}, facts, "ability:embrace-without-budget")
	var tool_candidate := _candidate(frontier, "tool:charm-scream-tail")
	var tool_annotation: Dictionary = _module_annotation(
		tool_candidate, "gardevoir_embrace").get("prize_scaler_tool", {})
	var certificate_exact := bool(tool_annotation.get("crosses_public_ko_threshold", false)) \
		and bool(tool_annotation.get("wins_now_after_public_embrace_sequence", false)) \
		and int(tool_annotation.get("required_assignments", 0)) == 6 \
		and int(tool_annotation.get("projected_damage", 0)) == 240 \
		and str(_route_safety(tool_candidate, frontier, facts).get("reason", "")) == "module_verified_advantage"

	var no_tool_processor := EffectProcessor.new()
	var no_tool_state := _game_state()
	var no_tool_tail := _real_slot(_real_card_data(SCREAM_TAIL_UID), 0)
	var no_tool_gardevoir := _real_slot(_real_card_data(GARDEVOIR_UID), 0)
	var no_tool_fuel: Array[CardInstance] = []
	for index: int in 5:
		no_tool_fuel.append(_real_instance(PSYCHIC_UID, 0))
	no_tool_state.players[0].active_pokemon = no_tool_tail
	no_tool_state.players[0].bench = [no_tool_gardevoir]
	no_tool_state.players[0].discard_pile = no_tool_fuel.duplicate()
	no_tool_state.players[1].active_pokemon = _real_target("Public no-tool 230 HP target", 230, 2)
	no_tool_processor.register_pokemon_card(no_tool_tail.get_card_data())
	no_tool_processor.register_pokemon_card(no_tool_gardevoir.get_card_data())
	var no_tool_exact := true
	for index: int in 4:
		no_tool_exact = no_tool_exact and no_tool_processor.execute_ability_effect(
			no_tool_gardevoir, 0, [{
				"embrace_energy": [no_tool_fuel[index]],
				"embrace_target": [no_tool_tail],
			}], no_tool_state)
	var no_tool_effect := no_tool_processor.get_ability_effect(no_tool_gardevoir, 0, no_tool_state)
	var no_tool_budget_stops_at_160 := no_tool_exact and no_tool_tail.damage_counters == 80 \
		and no_tool_effect != null \
		and not bool(no_tool_effect.call("can_use_embrace_on_target", no_tool_tail, no_tool_state))
	...
	var target_step := _step(
		embrace_effect.get_interaction_steps(gardevoir.get_top_card(), state) if embrace_effect != null else [],
		"embrace_target")
	var target_now_legal := scream_tail in (target_step.get("items", []) as Array)
	var all_six_attached := true
	for index: int in 6:
		all_six_attached = all_six_attached and processor.execute_ability_effect(
			gardevoir, 0, [{
				"embrace_energy": [psychic_fuel[index]],
				"embrace_target": [scream_tail],
			}], state)
	var stopped_at_exact_budget := scream_tail.damage_counters == 120 \
		and scream_tail.attached_energy.size() == 6 \
		and processor.get_effective_max_hp(scream_tail, state) == 140 \
		and processor.get_effective_remaining_hp(scream_tail, state) == 20 \
		and not bool(embrace_effect.call("can_use_embrace_on_target", scream_tail, state))
	var attack_legal := RuleValidator.new().can_use_attack(state, 0, 1, processor)
	var attack_damage := _scream_tail_preview(
		processor, scream_tail, state.players[1].active_pokemon, state)
	var strict_hp_boundary := attack_damage == 240 and attack_damage >= 230 and attack_damage < 241

	var passed := certificate_exact and no_tool_budget_stops_at_160 and target_now_legal \
		and all_six_attached and stopped_at_exact_budget and attack_legal and strict_hp_boundary
	_check(passed, "scenario A must use Charm before exactly six Embraces for Roaring Scream 240: %s" % JSON.stringify({
		"certificate_exact": certificate_exact,
		"tool_annotation": tool_annotation,
		"no_tool_budget_stops_at_160": no_tool_budget_stops_at_160,
		"target_now_legal": target_now_legal,
		"all_six_attached": all_six_attached,
		"damage": scream_tail.damage_counters,
		"energy_count": scream_tail.attached_energy.size(),
		"remaining_hp": processor.get_effective_remaining_hp(scream_tail, state),
		"attack_legal": attack_legal,
		"attack_damage": attack_damage,
	}))
	_rows.append(_row(
		"charm_before_exact_six_embraces",
		"精神拥抱精确填能与伤害预算",
		"吼叫尾面对230HP双奖目标时，先贴勇气护符，再精确精神拥抱6次，停在120伤害/20HP并打出240；无护符只能安全推进到80伤害/160输出。",
		"Bravery Charm -> Psychic Embrace x6 -> Roaring Scream 240",
		["无护符不能完成6次拥抱", "120伤害后剩余20HP不得第7次拥抱", "240不能击倒241HP"],
		passed
	))
...
func _scenario_b_rare_candy_resolves_the_old_ralts_chain() -> void:
	...
	before["observation_hash"] = "standard-before-candy"
	...
	after["observation_hash"] = "standard-after-candy"
...
func _scenario_c_iono_before_fezandipiti_preserves_both_draws() -> void:
	...
	before["observation_version"] = 1
	before["observation_hash"] = "standard-before-iono"
	...
	after["observation_version"] = 2
	after["observation_hash"] = "standard-after-iono"
...
func _scenario_d_adrena_brain_moves_only_the_lethal_counters() -> void:
	...
	_fill_prizes(state.players[0], 2, "OWN_TWO_PRIZE_ROUTE")
	state.players[1].active_pokemon = _real_target("Public 190 HP Active", 190, 1)
	...
	var followup: Array = ability_effect.get_followup_interaction_steps(
		munkidori.get_top_card(), state, {"source_pokemon": [gardevoir]}
	) if ability_effect != null else []
	var counter_step := _step(followup, "target_damage_counters")
	var interaction_exact := gardevoir in (source_step.get("items", []) as Array) \
	...
	var unchanged_after_rejection := gardevoir.damage_counters == 30 and bench_target.damage_counters == 0
	var moved := processor.execute_ability_effect(munkidori, 0, [{
		"source_pokemon": [gardevoir],
		"target_damage_counters": [{"target": bench_target, "amount": 20}],
	}], state)
	var exact_lethal_transfer := gardevoir.damage_counters == 10 \
	...
	var fixed_attack_still_legal := RuleValidator.new().can_use_attack(state, 0, 0, processor)
	var fixed_attack_damage := DamageCalculator.new().calculate_damage(
		gardevoir,
		state.players[1].active_pokemon,
		gardevoir.get_card_data().attacks[0],
		state,
		_attack_bonus(processor, gardevoir, 0, state)
	)
	var munkidori_then_attack_closes_two := fixed_attack_still_legal \
		and fixed_attack_damage == 190 \
		and _prize_count(bench_target) + _prize_count(state.players[1].active_pokemon) >= state.players[0].prizes.size()

	var observation := _observation(
		[
			_ability("ability:adrena-bench-lethal", "slot:munkidori", MUNKIDORI_UID, true),
			_attack("attack:gardevoir-active-now", GARDEVOIR_UID, "slot:gardevoir", 0, 190, true),
		],
		_slot("slot:gardevoir", GARDEVOIR_UID, [_psychic_energy(), _psychic_energy(), _psychic_energy()], 30, 280, 310, 2),
		[_slot("slot:munkidori", MUNKIDORI_UID, [_darkness_energy()], 0, 110, 110, 1)],
		_public_target("PUBLIC_190_HP_ACTIVE", 190, 1),
		[_public_target("PUBLIC_20_HP_BENCH", 20, 1)],
		18,
		2
	)
	var facts := _facts(true, true, false, 0, false, false, 190, 2)
	var frontier := _frontier(observation, {
		"ability:adrena-bench-lethal": 610.0,
		"attack:gardevoir-active-now": 590.0,
	...
	var public_budget_visible: bool = int(counter_annotation.get("movable_counter_budget", 0)) == 3 \
		and "bind_counter_source_and_target" in counter_annotation.get("decision_hints", []) \
		and not bool(counter_annotation.get("counter_mover_before_secured_ko", {}).get(
			"preserves_secured_prize_suffix", false))

	var attack_processor := EffectProcessor.new()
	var attack_state := _game_state()
	var attack_tail := _real_slot(_real_card_data(SCREAM_TAIL_UID), 0)
	attack_tail.damage_counters = 60
	attack_tail.attached_energy = [_real_instance(PSYCHIC_UID, 0), _real_instance(PSYCHIC_UID, 0)]
	var attack_munkidori := _real_slot(_real_card_data(MUNKIDORI_UID), 0)
	attack_munkidori.attached_energy = [_real_instance(DARKNESS_UID, 0)]
	attack_state.players[0].active_pokemon = attack_tail
	attack_state.players[0].bench = [attack_munkidori]
	var attack_target := _real_target("Public exact 120 HP Active", 120, 1)
	var nonlethal_counter_target := _real_target("Public 40 HP Bench", 40, 1)
	attack_state.players[1].active_pokemon = attack_target
	attack_state.players[1].bench = [nonlethal_counter_target]
	attack_processor.register_pokemon_card(attack_tail.get_card_data())
	attack_processor.register_pokemon_card(attack_munkidori.get_card_data())
	var attack_before_move := _scream_tail_preview(
		attack_processor, attack_tail, attack_target, attack_state)
	var nonlethal_move := attack_processor.execute_ability_effect(attack_munkidori, 0, [{
		"source_pokemon": [attack_tail],
		"target_damage_counters": [{"target": nonlethal_counter_target, "amount": 30}],
	}], attack_state)
	var attack_after_move := _scream_tail_preview(
		attack_processor, attack_tail, attack_target, attack_state)
	var attack_now_branch_exact := attack_before_move == 120 \
		and nonlethal_move and nonlethal_counter_target.get_remaining_hp() == 10 \
		and attack_after_move == 60 and attack_after_move < attack_target.get_remaining_hp()
	...
	var passed: bool = interaction_exact and too_many_rejected and unchanged_after_rejection \
		and moved and exact_lethal_transfer and once_per_turn and munkidori_then_attack_closes_two \
		and public_budget_visible and attack_now_branch_exact and no_dark_blocked
	_check(passed, "scenario D must choose Munkidori only when the transfer preserves the following attack: %s" % JSON.stringify({
		"interaction_exact": interaction_exact,
		"too_many_rejected": too_many_rejected,
		"unchanged_after_rejection": unchanged_after_rejection,
		"moved": moved,
		"exact_lethal_transfer": exact_lethal_transfer,
		"once_per_turn": once_per_turn,
		"fixed_attack_still_legal": fixed_attack_still_legal,
		"fixed_attack_damage": fixed_attack_damage,
		"munkidori_then_attack_closes_two": munkidori_then_attack_closes_two,
		"public_budget_visible": public_budget_visible,
		"attack_before_move": attack_before_move,
		"attack_after_move": attack_after_move,
		"attack_now_branch_exact": attack_now_branch_exact,
		"no_dark_blocked": no_dark_blocked,
	}))
	_rows.append(_row(
		"attack_or_adrena_brain_route",
		"攻击或愿增猿路线",
		"沙奈朵ex的190固定伤害不会因搬走自身伤害而下降，因此先由愿增猿精确搬20击倒备战，再攻击190HP前台可闭合两奖；吼叫尾的伤害会随自身伤害下降，搬30却不能击倒40HP目标时必须直接攻击。",
		"Munkidori 20-counter KO -> Gardevoir ex 190, otherwise Roaring Scream now",
		["愿增猿没有恶能量时不能使用特性", "单次最多搬30伤害", "不能为非击倒搬运而破坏吼叫尾现成击倒", "未绑定交互目标时模块不得伪造确定性证书"],
		passed
	))
...
func _observation(
	...
	return {
		...
		"observation_hash": "standard-gardevoir-complex-scenario",
...
func _scream_tail_preview(
	...
	for effect: BaseEffect in processor.get_attack_effects_for_slot(attacker, 1):
		if effect.has_method("get_attack_preview_damage"):
			return int(effect.call("get_attack_preview_damage", attacker, target, state))
	return 0
...
func _write_report() -> void:
	...
	var report := {
		...
		"known_production_gaps": [
			...
			"The Bravery Charm plus exact six-Embrace Roaring Scream suffix is real-effect verified and locally certified, but each post-interaction information epoch still has to bind the exact target in a live run.",
		...
		"coverage": [
			"Bravery Charm before six safe Psychic Embrace assignments for Roaring Scream 240",
			...
			"Gardevoir fixed-damage Munkidori two-Prize route versus Scream Tail attack-now branch",
